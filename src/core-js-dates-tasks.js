...
function getNextFriday(date) {
  ...
  const days = (5 - dayName + 7) % 7 || 7;
  return new Date(currentDate.setUTCDate(currentDate.getUTCDate() + days));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (endDay - startDay) / msInDay + 1;
}
...
function isLeapYear(date) {
  ...
  const leapYear = currentYear % 4 === 0 && currentYear % 100 !== 0;
  return leapYear || currentYear % 400 === 0;
}
